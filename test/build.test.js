...
test('adds runnable ad-hoc settings to unsigned macOS builds', () => {
  assert.deepEqual(
    ...
    ['--dir', '--mac', '--arm64', '--config.mac.identity=-', '--config.mac.hardenedRuntime=false']
  )
})

test('preserves hardened settings when a signing identity is configured', () => {
  const args = ['--mac', '--universal']
  assert.deepEqual(buildArguments(args, 'darwin', { CSC_LINK: 'certificate' }), args)
...
test('does not add macOS settings to Windows or Linux builds', () => {
  assert.deepEqual(buildArguments(['--win', '--x64'], 'win32', {}), ['--win', '--x64'])
  assert.deepEqual(buildArguments(['--linux', '--x64'], 'linux', {}), ['--linux', '--x64'])
})
